...
const marketSubmit = (e) => {
 ...
 axios.post("/api/createMarket", formData).then(res=>{
  if (res.data.status === 200) {
    swal("Success", res.data.message, "success");
    navigate('/buyer/dashboard/marketplace/:id')
  ...
  return (
    <div className='container py-5'>
        <div className="card" style={{display: 'flex',  justifyContent:'center', width:"800px", left:"200px", top:"20px"}}>
        <div
          className="card-body"
          style={{
            display: "flex",
        ...
        >
          ...
          <span className="login100-form-title">Create Marketplace</span>
            ...
            <div
              className="wrap-input100 validate-input">
                <label style={{fontSize:"15px"}} className="form-control-label px-3">Description
                        <span className="text-danger"> *</span>
                        </label>
                <textarea
                className="input100"
            ...
            <button type="button"  className="btn btn-success ">
              <input style={{width:'300px', height:'30px', fontSize:'14px'}}
               type="file"  name='image' onChange={handleImage} />
            </button>
